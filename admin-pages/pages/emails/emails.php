...
class HbAdminPageEmails extends HbAdminPage {
	...
	public function __construct( $page_id, $hbdb, $utils, $options_utils ) {
		...
		$this->email_actions = apply_filters(
			...
			array(
				...
				array(
					'action_value' => 'new_later_payment',
					'action_text' => esc_html__( 'Payment via Payment link', 'hbook-admin' ),
				),
				array(
					'action_value' => 'store_later_payment',
					'action_text' => esc_html__( 'New payment method via Payment link', 'hbook-admin' ),
				),
			)
		);
		...
		$this->data = array(
			'hb_text' => array(
				...
				'payment_link_text_format_error' => esc_html__( 'Payment links can not be used in TEXT format emails.', 'hbook-admin' ),
			),
	...
	<div class="wrap">
		...
		<p>
			<?php echo( sprintf( esc_html__( 'We strongly recommend using %s to send emails, in order to increase email deliverability. Please check %s this article from our knowledgebase %s.', 'hbook-admin' ), '<b>SMTP</b>', '<a href="https://maestrel-resource-center.refined.site/space/KB/63177009/" target="_blank">', '</a>' ) ); ?>
		</p>
		<br/>
		...
		<h3><?php esc_html_e( 'Email headers', 'hbook-admin' ); ?></h4>
		...
		<h3><?php esc_html_e( 'Variables', 'hbook-admin' ); ?></h3>
		<p>
			<?php esc_html_e( 'You can use the following variables in the email headers, subject and body.', 'hbook-admin' ); ?><br/>
			<?php echo( sprintf( esc_html__( 'Please check %s this section %s of our documentation to have a description of each variable.', 'hbook-admin' ), '<a href="https://maestrel-resource-center.refined.site/space/HD/34668701/" target="_blank">', '</a>' ) ); ?>
		</p>
		<?php $this->display_ical_email_document_vars(); ?>

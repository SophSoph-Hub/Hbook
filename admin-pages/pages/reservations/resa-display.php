...
class HbAdminPageReservationsDisplayHelper {
	...
	private $payment_link_types;

	public function __construct( $accom_list_info, $email_templates, $document_templates, $is_site_multi_lang, $site_langs, $is_resa_customer, $currency_symbol, $site_has_extras, $payment_link_types ) {
		...
		$this->payment_link_types = $payment_link_types;
	}
	...
	<div class="hb-resa-section">
		...
		<p data-bind="visible: resa_detailed_all_children_link_visible">
			<a href="#" data-bind="click: resa_detailed_show_all_children"><?php esc_html_e( 'View all reservations included in this Multiple reservation', 'hbook-admin' ); ?></a>
		</p>
		<!-- /ko -->
	...
	?>
		...
		<tr data-bind="attr: { 'data-resa-num': id, class: anim_class() + ' ' + parent_child_class }">
			...
			<td class="hb-resa-price-column" data-label="<?php esc_attr_e( 'Price / Payment', 'hbook-admin' ); ?>">
				...
				<div data-bind="visible: paid() != -1 && ! is_child">
					<div data-bind="visible: paid() != 0, html: price_details"></div>
					<?php if ( $this->user_can_edit() ) { ?>
					<div>
						<a data-bind="click: $root.edit_payments" href="#"><?php esc_html_e( 'Edit payments', 'hbook-admin' ); ?></a>
					</div>
					<?php } else if ( $this->user_can_view( 'read_resa_payments' ) ) { ?>
					<div>
						<a data-bind="click: $root.edit_payments" href="#"><?php esc_html_e( 'View payments', 'hbook-admin' ); ?></a>
					</div>
					<?php } ?>
					<div data-bind="visible: editing_payments()" class="hb-payments-logs-wrapper">
						<a href="#" class="dashicons dashicons-dismiss" data-bind="click: $root.close_payments_logs"></a>
						<?php if ( $this->user_can_edit() ) { ?>
						<div class="hb-add-payment-wrapper">
							<div><b><?php esc_html_e( 'Add a payment', 'hbook-admin' ); ?></b></div>
							<hr/>
							<div class="hb-clearfix">
								<a data-bind="visible: unpaid() > 0, click: $root.insert_unpaid_amount" class="hb-add-payment-custom-amount" href="#">
									<?php esc_html_e( 'Remaining balance', 'hbook-admin' ); ?> <span>(<span data-bind="text: hb_decode_entities( hb_format_price( unpaid() ) )"></span>)</span>
								</a>
								<a data-bind="visible: ! past() && hb_paid_security_bond == 'yes' && hb_security_bond > 0 && paid() < hb_security_bond, click: $root.insert_security_bond_amount" class="hb-add-payment-custom-amount" href="#">
									<?php esc_html_e( 'Security bond', 'hbook-admin' ); ?> <span>(<span data-bind="text: hb_decode_entities( hb_format_price( hb_security_bond ) )"></span>)</span>
								</a>
								<a data-bind="visible: paid() > 0, click: $root.insert_full_refund_amount" class="hb-add-payment-custom-amount" href="#">
									<?php esc_html_e( 'Full refund', 'hbook-admin' ); ?> <span>(<span data-bind="text: hb_decode_entities( hb_format_price( paid() ) )"></span>)</span>
								</a>
								<a data-bind="visible: stripe_max_refundable() > 0 && stripe_max_refundable() != paid(), click: $root.insert_full_stripe_refund_amount" class="hb-add-payment-custom-amount" href="#">
									<?php esc_html_e( 'Full Stripe refund', 'hbook-admin' ); ?> <span>(<span data-bind="text: hb_decode_entities( hb_format_price( stripe_max_refundable() ) )"></span>)</span>
								</a>
								<a data-bind="visible: hb_paid_security_bond == 'yes' && hb_security_bond > 0 && paid() > hb_security_bond, click: $root.insert_security_bond_refund_amount" class="hb-add-payment-custom-amount" href="#">
									<?php esc_html_e( 'Security bond refund', 'hbook-admin' ); ?> <span>(<span data-bind="text: hb_decode_entities( hb_format_price( hb_security_bond ) )"></span>)</span>
								</a>
								<div class="hb-add-payment-column">
									<label data-bind="attr: { for: 'hb-add-payment-received-on-' + id }"><?php esc_html_e( 'Received on', 'hbook-admin' ); ?></label>
									<input class="hb-input-payment-received-on" data-bind="value: payment_received_on_input, attr: { id: 'hb-add-payment-received-on-' + id }" type="text" />
								</div>
								<div class="hb-add-payment-column">
									<label data-bind="attr: { for: 'hb-add-payment-amount-' + id }"><?php esc_html_e( 'Amount', 'hbook-admin' ); ?></label>
									<input data-bind="textInput: payment_amount, attr: { id: 'hb-add-payment-amount-' + id }" type="text" />
								</div>
								<div class="hb-add-payment-column">
									<label>
										<?php esc_html_e( 'Method', 'hbook-admin' ); ?>
										<span data-bind="visible: hb_offline_payment_methods.length == 1 && ! charge_action_visible()">:</span>
									</label>
									<!-- ko foreach: hb_offline_payment_methods -->
									<input data-bind="visible: hb_offline_payment_methods.length > 1 || $parent.charge_action_visible, checked: $parent.payment_method, attr: { id: input_id + '-' + $parent.id, value: input_value }" type="radio" />
									<label data-bind="attr: { for: input_id + '-' + $parent.id }, text: label_text"></label><br/>
									<!-- /ko -->
									<!-- ko if: charge_action_visible() -->
									<input data-bind="checked: payment_method, attr: { id: 'hb-add-payment-method-stripe-' + id }" type="radio" value="Stripe" />
									<label data-bind="attr: { for: 'hb-add-payment-method-stripe-' + id }">Stripe</label><br/>
									<!-- /ko -->
								</div>
								<div class="hb-add-payment-column">
									<label data-bind="attr: { for: 'hb-add-payment-comment-' + id }"><?php esc_html_e( 'Comment', 'hbook-admin' ); ?></label>
									<textarea data-bind="value: payment_comment, attr: { id: 'hb-add-payment-comment-' + id }" rows="2"></textarea>
								</div>
							</div>
							<!-- ko if: payment_method() != 'Stripe' -->
							<a data-bind="visible: ! saving_payment(), click: $root.add_payment" href="#" class="button-primary">
								<span data-bind="visible: payment_amount_is_positive()"><?php esc_html_e( 'Add payment', 'hbook-admin' ); ?></span>
								<span data-bind="visible: ! payment_amount_is_positive()"><?php esc_html_e( 'Add refund', 'hbook-admin' ); ?></span>
							</a>
							<a data-bind="visible: saving_payment()" disabled href="#" class="button-primary">
								<span data-bind="visible: payment_amount_is_positive()"><?php esc_html_e( 'Adding payment...', 'hbook-admin' ); ?></span>
								<span data-bind="visible: ! payment_amount_is_positive()"><?php esc_html_e( 'Adding refund...', 'hbook-admin' ); ?></span>
							</a>
							<!-- /ko -->
							<!-- ko if: payment_method() == 'Stripe' && charge_action_visible() -->
							<!-- ko if: payment_amount_is_positive() -->
							...
							<!-- /ko -->
							<!-- ko if: ! payment_amount_is_positive() -->
							<a data-bind="click: $root.refund, visible: ! refunding() && refund_action_visible()" href="#" class="button-primary"><?php esc_html_e( 'Refund', 'hbook-admin' ); ?></a>
							<input type="button" disabled data-bind="visible: ! refund_action_visible()" class="button-primary" value="<?php esc_attr_e( 'Refund', 'hbook-admin' ); ?>" />
							<input type="button" disabled data-bind="visible: refunding()" class="button-primary" value="<?php esc_attr_e( 'Refunding...', 'hbook-admin' ); ?>" />
							<!-- /ko -->
							<!-- /ko -->
						</div><!-- end .hb-add-payment-wrapper -->
						<?php } ?>
						<?php if ( $this->user_can_view( 'read_resa_payments' ) ) { ?>
						<!-- ko if: payments_logs().length > 0 -->
						<table class="hb-payments-logs">
							<thead>
								<tr>
									<th class="hb-payment-log-id-th"><?php esc_html_e( 'Id', 'hbook-admin' ); ?></th>
									<th><?php esc_html_e( 'Received on', 'hbook-admin' ); ?></th>
									<th><?php esc_html_e( 'Amount', 'hbook-admin' ); ?></th>
									<th><?php esc_html_e( 'Method', 'hbook-admin' ); ?></th>
									<th><?php esc_html_e( 'Comment', 'hbook-admin' ); ?></th>
									<th class="hb-delete-payment-log-th"></th>
								</tr>
							</thead>
							<tbody data-bind="foreach { data: payments_logs }">
								<tr>
									<td class="hb-payments-logs-td" data-bind="text: id"></td>
									<td class="hb-payments-logs-td" data-bind="text: hb_formatted_date( received_on )"></td>
									<td class="hb-payments-logs-td" data-bind="text: hb_decode_entities( hb_format_price( amount ) )"></td>
									<td class="hb-payments-logs-td" data-bind="text: payment_method"></td>
									<td class="hb-payments-logs-td hb-payment-log-comment-td">
										<div data-bind="text: hb_decode_entities( comment )"></div>
										<div
											data-bind="visible: delayed"
											class="hb-payment-status hb-resa-payment-delayed"
											title="<?php esc_attr_e( 'Delayed payment', 'hbook-admin' ); ?>"
										>
											<?php esc_html_e( 'Delayed', 'hbook-admin' ); ?>
										</div>
										<div
											data-bind="visible: failed"
											class="hb-payment-status hb-resa-payment-failed"
											title="<?php esc_attr_e( 'Failed payment', 'hbook-admin' ); ?>"
										>
											<?php esc_html_e( 'Failed', 'hbook-admin' ); ?>
										</div>
									</td>
									<td class="hb-payments-logs-td hb-delete-payment-log-td">
										<?php if ( $this->user_can_edit() ) { ?>
										<!-- ko if: ( payment_method != 'Stripe' && online == 'no' ) || ( payment_method == 'Stripe' && failed ) -->
										<a
											href="#"
											title="<?php esc_attr_e( 'Delete payment', 'hbook-admin' ); ?>"
											class="dashicons dashicons-trash"
											data-bind="click: function() { $root.delete_payment( $parent, id ) }, visible: ! deleting_payment()">
										</a>
										<!-- /ko -->
										<!-- ko if: payment_method == 'Stripe' && ! refunded && ! delayed && ! failed -->
										<a
											href="#"
											title="<?php esc_attr_e( 'Refund payment', 'hbook-admin' ); ?>"
											class="dashicons dashicons-undo"
											data-bind="click: function() { $root.delete_payment( $parent, id ) }, visible: ! deleting_payment()">
										</a>
										<!-- /ko -->
										<span data-bind="visible: deleting_payment()" class="spinner"></span>
										<?php } ?>
									</td>
								</tr>
							</tbody>
						...
						<!-- /ko -->
						<?php } ?>
					</div><!-- end .hb-payments-logs-wrapper -->
				</div>
						...
						<table class="hb-email-logs">
							...
							<tbody data-bind="foreach { data: email_logs }">
								<tr>
									...
									<td class="hb-email-logs-td" data-bind="text: hb_formatted_date( sent_time )">
			...
			<td class="hb-resa-actions-column">
				...
				<div data-bind="visible: preparing_email()" class="hb-sending-email-wrapper">
					...
					<?php if ( get_option( 'hb_stripe_active' ) == 'yes' ) { ?>
					<p><a href="#" data-bind="click: $root.open_close_payment_link"><?php esc_html_e( 'Add payment link', 'hbook-admin' ); ?></a></p>
					<?php } ?>
					<div class="hb-insert-payment-link-wrapper" data-bind="visible: adding_payment_link">
						<?php foreach ( $this->payment_link_types as $payment_link_type => $payment_link_type_label ) { ?>
						<p class="hb-payment-link-type">
							<input
								data-bind="checked: payment_link_type, attr: {id: 'hb-payment-link-<?php echo( $payment_link_type ); ?>-' + id }"
								type="radio"
								value="<?php echo( $payment_link_type ); ?>"
							/>
							<label data-bind="attr: {for: 'hb-payment-link-<?php echo( $payment_link_type ); ?>-' + id }"><?php echo( $payment_link_type_label ); ?></label>
						</p>
						<?php } ?>
						<p class="hb-payment-link-custom-amount" data-bind="visible: payment_link_type() == 'custom_amount'">
							<label data-bind="attr: {for: 'hb-payment-link-custom-amount-' + id }"><?php esc_html_e( 'Amount:', 'hbook-admin' ); ?></label>
							<input data-bind="attr: {id: 'hb-payment-link-custom-amount-' + id }, value: payment_link_custom_amount" type="text" />
						</p>
						<p class="hb-payment-link-insert-button-wrapper">
							<a data-bind="click: $root.insert_payment_link" href="#" class="button"><?php esc_html_e( 'Insert', 'hbook-admin' ); ?></a>
						</p>
					</div>

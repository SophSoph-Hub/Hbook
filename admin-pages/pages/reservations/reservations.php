...
class HbAdminPageReservations extends HbAdminPage {
	...
	public function __construct( $page_id, $hbdb, $utils, $options_utils ) {
		$hb_text = array(
			...
			'payments_delayed' => esc_html__( 'Delayed payments', 'hbook-admin' ),
			'payments_failed' => esc_html__( 'Failed payments', 'hbook-admin' ),
			...
			'charge_amount_too_high' => esc_html__( 'The charge amount is above the amount due by the customer (%amount). Do you wish to proceed?', 'hbook-admin' ),
			'charge_amount_negative' => esc_html__( 'The charge amount can not be null or below zero.', 'hbook-admin' ),
			'refund_amount_too_high' => esc_html__( 'The refund amount can not be above %amount', 'hbook-admin' ),
			'refund_amount_positive' => esc_html__( 'The amount should be negative.', 'hbook-admin' ),
			...
			'confirm_delete_payment' => esc_html__( 'Delete payment?', 'hbook-admin' ),
			'confirm_refund_payment' => esc_html__( 'Refund payment?', 'hbook-admin' ),
			'invalid_amount' => esc_html__( 'Invalid amount.', 'hbook-admin' ),
			'payment_comment_unpaid_amount' => esc_html__( 'Remaining balance', 'hbook-admin' ),
			'payment_comment_security_bond_amount' => esc_html__( 'Security bond', 'hbook-admin' ),
			'payment_comment_full_refund_amount' => esc_html__( 'Full refund', 'hbook-admin' ),
			'payment_comment_full_stripe_refund_amount' => esc_html__( 'Full Stripe refund', 'hbook-admin' ),
			'payment_comment_security_bond_refund_amount' => esc_html__( 'Security bond refund', 'hbook-admin' ),
		...
		foreach ( $resa as $key => $resa_data ) {
			...
			$resa[ $key ]['payments_logs'] = $utils->prepare_resa_payments_logs( $resa_data );
		...
		foreach ( $parents_resa as $key => $parent_resa_data ) {
			...
			$parents_resa[ $key ]['payments_logs'] = $utils->prepare_resa_payments_logs( $parent_resa_data );
		}
		...
		$offline_payment_methods = get_option( 'hb_offline_payment_methods' );
		if ( $offline_payment_methods ) {
			$offline_payment_methods_tmp = array();
			$offline_payment_methods = explode( ',', $offline_payment_methods );
			foreach ( $offline_payment_methods as $payment_method ) {
				$offline_payment_methods_tmp[] = array(
					'input_id' => esc_attr( 'hb-add-payment-method-' . str_replace( ' ', '-', trim( $payment_method ) ) ),
					'input_value' => esc_attr( trim( $payment_method ) ),
					'label_text' => esc_html( trim( $payment_method ) ),
				);
			}
			$offline_payment_methods = $offline_payment_methods_tmp;
		} else {
			$offline_payment_methods = array(
				array(
					'input_id' => 'hb-add-payment-method-default-offline',
					'input_value' => esc_attr__( 'Offline payment', 'hbook-admin' ),
					'label_text' => esc_html__( 'Offline payment', 'hbook-admin' ),
				)
			);
		}
		...
		$this->data = array(
			...
			'hb_offline_payment_methods' => $offline_payment_methods,
		...
		$this->resa_display_helper = new HbAdminPageReservationsDisplayHelper( $accom_info, $this->email_templates, $this->document_templates, $this->is_site_multi_lang, $this->site_langs, $this->is_resa_customer, $this->utils->get_currency_symbol(), $this->hbdb->site_has_extras(), $this->utils->get_payment_link_types() );
	}

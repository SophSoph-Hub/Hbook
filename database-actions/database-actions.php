...
class HbDataBaseActions {
	...
	public function __construct() {
		...
		$this->later_payments_table = $this->prefix . 'later_payments';
	}
	...
	public function update_resa_after_payment( $token, $payment_status, $payment_status_reason, $paid, $payment_info = false, $payment_delayed = false ) {
		...
		if ( $paid > 0 ) {
			$new_payment = array(
				...
				'comment' => '',
				'online' => 'yes',
				'received_on' => current_time( 'mysql', 1 ),
			);
			if ( $payment_info ) {
				$new_payment['gateway_info'] = $payment_info;
			}
			if ( $payment_delayed ) {
				$new_payment['delayed'] = 'yes';
				$new_values['payment_delayed'] = 1;
			}
			$new_values['payments_logs'] = json_encode( array( $new_payment ) );
		} else {
			$new_values['payments_logs'] = json_encode( array() );
		}
		$resa_id = $resa['id'];
	...
	public function update_resa_payment_delayed( $resa, $payments_logs, $resa_has_delayed_payment ) {
		$new_values = array(
			'payments_logs' => json_encode( $payments_logs ),
			'payment_delayed' => $resa_has_delayed_payment,
		);
		if ( ! isset( $resa['parent_id'] ) ) {
	...
	public function tag_resa_payment_delayed( $resa_id, $resa_is_parent ) {
		if ( $resa_is_parent ) {
			$update = $this->db->update( $this->parents_resa_table, array( 'payment_delayed' => 1 ), array( 'id' => $resa_id ) );
		} else {
			$update = $this->db->update( $this->resa_table, array( 'payment_delayed' => 1 ), array( 'id' => $resa_id ) );
		}
	}
	...
	private function get_uncompleted_resa_deletion_delay() {
		...
		if ( ( $delay_check[1] == 'MINUTE') && ( $delay_check[0] < 15 ) ) {
			$delay = '15 MINUTE';
		}
		return $delay;
	}

	public function get_uncompleted_parent_resa( $old = false, $return_ids = false ) {
		$request = "SELECT * FROM $this->resa_table WHERE ( parent_id != 0 ) AND ( status = 'waiting_payment' )";
		if ( $old ) {
			$delay = $this->get_uncompleted_resa_deletion_delay();
		} else {
			$delay = '15 MINUTE';
		}
		$request .=  " AND ( '" . current_time( 'mysql', 1 ) . "' - INTERVAL $delay > received_on)";
		$uncompleted_children_resa = $this->db->get_results( $request, ARRAY_A );
		$parent_resa_ids = array();
		foreach ( $uncompleted_children_resa as $child_resa ) {
			if ( ! in_array( $child_resa['parent_id'], $parent_resa_ids ) ) {
				$parent_resa_ids[] = $child_resa['parent_id'];
			}
 		}
		if ( $return_ids ) {
			return $parent_resa_ids;
		}
		$parent_resa = array();
		foreach ( $parent_resa_ids as $id ) {
			$parent_resa[] = $this->get_single( 'parents_resa', $id );
		}
		return $parent_resa;
	}

	public function get_old_uncompleted_parent_resa_ids() {
		return $this->get_uncompleted_parent_resa( true, true );
	}

	public function get_uncompleted_resa( $old = false, $return_ids = false ) {
		$request = "SELECT * FROM $this->resa_table WHERE ( status = 'waiting_payment' ) AND ( parent_id = 0 )";
		if ( $old ) {
			$delay = $this->get_uncompleted_resa_deletion_delay();
		}  else {
			$delay = '15 MINUTE';
		...
		$request .=  " AND ( '" . current_time( 'mysql', 1 ) . "' - INTERVAL $delay > received_on)";
		$resa = $this->db->get_results( $request, ARRAY_A );
		if ( $return_ids ) {
			$ids = array();
			foreach ( $resa as $r ) {
				$ids[] = $r['id'];
	...
	public function get_old_uncompleted_resa_ids() {
		return $this->get_uncompleted_resa( true, true );
	}
	...
	public function update_customer_country( $id, $country_info ) {
		$info = $this->get_customer_info( $id );
		$info['country_iso'] = $country_info['country_iso'];
		if ( $country_info['country_iso'] == 'US' ) {
			$info['usa_state_iso'] = $country_info['usa_state_iso'];
		}
		if ( $country_info['country_iso'] == 'CA' ) {
			$info['canada_province_iso'] = $country_info['canada_province_iso'];
		}
		$info = json_encode( $info );
		$this->db->update( $this->customers_table, array( 'info' => $info ), array( 'id' => $id ) );
	}
	...
	public function get_later_payment( $id ) {
		return $this->db->get_row(
			$this->db->prepare(
				"
				SELECT *
				FROM $this->later_payments_table
				WHERE alphanum_id = %s
				",
				$id
			)
	...
	public function get_later_payment_by_token( $token ) {
		return $this->db->get_row(
			$this->db->prepare(
				"
				SELECT *
	...
	public function add_later_payment( $resa_id, $resa_is_parent, $payment_type ) {
		$alphanum = $this->db->get_var(
			$this->db->prepare(
				"
				SELECT alphanum_id
				FROM $this->later_payments_table
				WHERE resa_id = %d AND resa_is_parent = %d AND payment_type = %s
				",
				$resa_id, $resa_is_parent, $payment_type
			)
		);
		if ( $alphanum ) {
			return $alphanum;
			...
			} while ( $this->get_later_payment( $alphanum ) );
			if (
				$this->db->insert(
					$this->later_payments_table,
					array(
						'alphanum_id' => $alphanum,
						'resa_id' => $resa_id,
						'resa_is_parent' => $resa_is_parent,
						'payment_type' => $payment_type,
						'created_on' => current_time( 'mysql', 1 ),
					)
				)
			) {
				return $alphanum;
	...
	public function update_later_payment_amount( $id, $amount_to_pay ) {
		return $this->db->update( $this->later_payments_table, array( 'amount_to_pay' => $amount_to_pay ), array( 'id' => $id ) );
	}

	public function update_later_payment_token( $id, $token ) {
		return $this->db->update( $this->later_payments_table, array( 'token' => $token ), array( 'id' => $id ) );
	}

	public function update_later_payment_status( $id, $status ) {
		return $this->db->update( $this->later_payments_table, array( 'status' => $status ), array( 'id' => $id ) );
	}

	public function update_resa_charges_info( $resa ) {
		if ( $resa['payment_info'] && ! $resa['payments_logs'] ) {
			$payment_info = json_decode( $resa['payment_info'], true );
			if (
				$payment_info &&
				isset( $payment_info['stripe_charges'] ) &&
				is_array( $payment_info['stripe_charges'] )
			) {
				$payments_logs = array();
				foreach ( $payment_info['stripe_charges'] as $id => $stripe_charge ) {
					$payments_logs[] = array(
						'id' => $id + 1,
			...
			$payment_info['legacy_stripe_charges'] = $payment_info['stripe_charges'];
			$payment_info['stripe_charges'] = 'updated';
			$payment_info = json_encode( $payment_info );
			$payments_logs = json_encode( $payments_logs );
			if ( ! isset( $resa['parent_id'] ) ) {
				$resa_table = $this->parents_resa_table;
			} else {
				$resa_table = $this->resa_table;
			}
			$this->db->update(
				$resa_table,
				array( 'payment_info' => $payment_info, 'payments_logs' => $payments_logs ),
				array( 'id' => $resa['id'] )
			);
			$resa['payment_info'] = $payment_info;
			$resa['payments_logs'] = $payments_logs;
		}
		return $resa;
	}

	public function add_resa_payment( $resa_id, $resa_is_parent, $new_payment ) {
		$payments = $this->get_resa_payments_logs( $resa_id, $resa_is_parent );
		if ( $payments ) {
			$payment_ids = array();
			...
			if ( $payment_ids ) {
				$new_payment['id'] = max( $payment_ids ) + 1;
			} else {
				$new_payment['id'] = 1;
			}
		} else {
		...
		$new_payment['refunded_amount'] = 0;
		if ( ! isset( $new_payment['comment'] ) ) {
			$new_payment['comment'] = '';
		}
		if ( ! isset( $new_payment['payment_method'] ) ) {
			$new_payment['payment_method'] = '';
		}
		if ( ! isset( $new_payment['received_on'] ) || ! $new_payment['received_on'] ) {
			$new_payment['received_on'] = current_time( 'mysql', 1 );
		}
		...
		if ( $updated !== false ) {
			if ( $resa_is_parent ) {
				do_action( 'hb_reservation_updated', 'paid', $resa_id );
			} else {
				do_action( 'hb_parent_reservation_updated', 'paid', $resa_id );
			}
			do_action( 'hb_reservations_updated' );
	...
	public function delete_resa_payment( $resa_id, $resa_is_parent, $payment_id ) {
		$payments = $this->get_resa_payments_logs( $resa_id, $resa_is_parent );
		...
		$updated_payments = json_encode( $updated_payments );
		if ( $resa_is_parent ) {
			$resa_table = $this->parents_resa_table;
		} else {
			$resa_table = $this->resa_table;
	...
	public function mark_resa_payment_refunded( $resa_id, $resa_is_parent, $payment_id, $refund_amount ) {
		$payments = $this->get_resa_payments_logs( $resa_id, $resa_is_parent );
		$updated_payments = array();
		foreach ( $payments as $payment ) {
			if ( $payment['id'] == $payment_id ) {
		...
		$this->db->update( $resa_table, array( 'payments_logs' => $updated_payments ), array( 'id' => $resa_id ) );
	}

	public function get_resa_payments_logs_raw( $resa_id, $resa_is_parent ) {
		if ( $resa_is_parent ) {
			$resa_table = $this->parents_resa_table;
		} else {
			$resa_table = $this->resa_table;
	...
	public function get_resa_payments_logs( $resa_id, $resa_is_parent ) {
		$payments_logs = $this->get_resa_payments_logs_raw( $resa_id, $resa_is_parent );
		if ( $payments_logs ) {
			return json_decode( $payments_logs, true );
		} else {
			return array();
		}
	}

...
class HbDataBaseCreation {
	...
	private function insert_fields() {
		...
		if ( $nb_fields == 0 ) {
			$details_form_title_label = esc_html__( 'Enter your details', 'hbook-admin' );
			$first_name_label = esc_html__( 'First name', 'hbook-admin' );
			$last_name_label = esc_html__( 'Last name', 'hbook-admin' );
			$email_label = esc_html__( 'Email', 'hbook-admin' );
			$phone_label = esc_html__( 'Phone', 'hbook-admin' );
			$address_1_label = esc_html__( 'Address line 1', 'hbook-admin' );
			$address_2_label = esc_html__( 'Address line 2', 'hbook-admin' );
			$city_label = esc_html__( 'City', 'hbook-admin' );
			$zip_code_label = esc_html__( 'Zip code', 'hbook-admin' );
			$country_iso_label = esc_html__( 'Country', 'hbook-admin' );
			$state_province_label = esc_html__( 'State / province', 'hbook-admin' );
			$query = "
				INSERT INTO {$this->hbdb->fields_table}
				(id, name, standard, displayed, required, type, order_num, data_about)
				VALUES
				('details_form_title', '$details_form_title_label', 1, 1, 0, 'title', 1, 'customer'),
				('first_name', '$first_name_label', 1, 1, 1, 'text', 2, 'customer'),
				('last_name', '$last_name_label', 1, 1, 1, 'text', 3, 'customer'),
				('email', '$email_label', 1, 1, 1, 'email', 4, 'customer'),
				('phone', '$phone_label', 0, 1, 0, 'text', 5, 'customer'),
				('address_1', '$address_1_label', 0, 1, 0, 'text', 6, 'customer'),
				('address_2', '$address_2_label', 0, 1, 0, 'text', 7, 'customer'),
				('city', '$city_label', 0, 1, 0, 'text', 8, 'customer'),
				('zip_code', '$zip_code_label', 0, 1, 0, 'text', 9, 'customer'),
				('country_iso', '$country_iso_label', 1, 1, 0, 'country_select', 10, 'customer'),
				('state_province', '$state_province_label', 1, 1, 0, 'text', 11, 'customer')
				";
			$wpdb->query( $query );
		}
	}
...
"</html>\n"
	...
	private function insert_strings_v_1_7() {
		...
		$query =
			...
			('stripe_processing_error', 'en_US', 'Your payment could not be processed (%error_msg).' )
			";
		$wpdb->query( $query );
	}
	...
	public function insert_strings_v_2_1() {
		global $wpdb;
		$query =
			"
			INSERT INTO {$this->hbdb->strings_table}
			(id, locale, value)
			VALUES
			('verifying_coupon', 'en_US', 'Verifying coupon...' ),
			('later_payment_form_could_not_be_loaded', 'en_US', 'The Payment form could not be loaded (you did not follow a Payment link or the Payment has expired).' ),
			('later_payment_already_paid', 'en_US', 'A payment has already been made for this Payment link.' ),
			('later_payment_form_title', 'en_US', 'Payment' ),
			('later_payment_type_explanation', 'en_US', 'You are paying the %payment_type.'),
			('later_payment_type_total_price', 'en_US', 'total price'),
			('later_payment_type_total_price_including_bond', 'en_US', 'total price including the bond'),
			('later_payment_type_deposit', 'en_US', 'deposit'),
			('later_payment_type_deposit_including_bond', 'en_US', 'deposit including the bond'),
			('later_payment_type_remaining_balance', 'en_US', 'remaining balance'),
			('later_payment_type_remaining_balance_including_bond', 'en_US', 'remaining balance including the bond'),
			('later_payment_type_bond', 'en_US', 'bond'),
			('later_payment_form_amount', 'en_US', 'You will be charged %amount' ),
			('later_payment_form_store_credit_card_text_explanation', 'en_US', 'We ask for your credit card details but we will not charge it now.' ),
			('later_payment_form_pay_button', 'en_US', 'Pay now' ),
			('later_payment_form_submit_button', 'en_US', 'Submit' ),
			('later_payment_txt_before_pay_button', 'en_US', 'Please double check the payment details before clicking on \"Pay now\"' ),
			('later_payment_txt_before_submit_button', 'en_US', 'Please double check the form details before clicking on \"Submit\"' ),
			('later_payment_thanks_message', 'en_US', 'Thank you. We have just sent you an email at %customer_email to confirm we have received your payment.' ),
			('later_payment_store_credit_card_thanks_message', 'en_US', 'Thank you. We have updated your payment method.' ),
			('stripe_payment_declined', 'en_US', 'Your payment was declined. Please try again and get in touch with us if you need help.' ),
			('table_invoice_bond', 'en_US', 'Security bond'),
			('table_invoice_due', 'en_US', 'Total due'),
			('table_invoice_total_with_bond', 'en_US', 'Total including security bond'),
			('table_invoice_paid', 'en_US', 'Amount paid'),
			('table_payments_history_total_paid', 'en_US', 'Total amount paid:'),
			('table_payments_history_head_date', 'en_US', 'Payment date'),
			('table_payments_history_head_method', 'en_US', 'Payment method'),
			('table_payments_history_head_amount', 'en_US', 'Amount'),
			('summary_resa_details_subtitle', 'en_US', 'Reservation details'),
			('summary_price_breakdown_subtitle', 'en_US', 'Price breakdown'),
			('summary_payments_history_subtitle', 'en_US', 'Payments history')
			";
		$wpdb->query( $query );
	}

	public function insert_strings_v_2_1_1() {
		global $wpdb;
		$query =
			"
			INSERT INTO {$this->hbdb->strings_table}
			(id, locale, value)
			VALUES
			('later_payment_pay_button_pay_txt', 'en_US', 'Pay now' ),
			('later_payment_update_button_txt', 'en_US', 'Add or update your payment method' )
			";
		$wpdb->query( $query );
	}
}
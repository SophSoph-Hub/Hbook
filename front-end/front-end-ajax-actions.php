...
class HbFrontEndAjaxActions {
	...
	public function hb_get_available_accom() {
		...
		$strings = $this->utils->get_strings( sanitize_text_field( $_POST['is_admin'] ) );
		...
		$search_request = array(
			...
			'page_accom_id' => sanitize_text_field( $_POST['page_accom_id'] ),
			'current_page_id' => sanitize_text_field( $_POST['current_page_id'] ),
			'exists_main_booking_form' => sanitize_text_field( $_POST['exists_main_booking_form'] ),
			'results_show_only_accom_id' => sanitize_text_field( $_POST['results_show_only_accom_id'] ),
			'force_display_thumb' => sanitize_text_field( $_POST['force_display_thumb'] ),
			'force_display_desc' => sanitize_text_field( $_POST['force_display_desc'] ),
			'is_admin' => sanitize_text_field( $_POST['is_admin'] ),
			'admin_accom_id' => sanitize_text_field( $_POST['admin_accom_id'] ),
			'admin_search_type' => sanitize_text_field( $_POST['admin_search_type'] ),
			'accom_people' => sanitize_text_field( $_POST['accom_people'] ),
		);
	...
	public function hb_process_later_payment() {
		$response = array();
		$payment_gateway = $this->utils->get_payment_gateway( $_POST['hb-payment-gateway'] );
		$payment = $this->hbdb->get_later_payment( $_POST['hb-payment-alphanum-id'] );
		if ( $payment_gateway && $payment ) {
			if ( $payment['resa_is_parent'] ) {
				$resa = $this->hbdb->get_single( 'parents_resa', $payment['resa_id'] );
				$resa_children = $this->hbdb->get_resa_by_parent_id( $payment['resa_id'] );
				if ( count( $resa_children ) >= 1 ) {
					$resa['check_in'] = $resa_children[0]['check_in'];
					$resa['check_out'] = $resa_children[0]['check_out'];
				}
			} else {
			...
			if ( $resa ) {
				...
				if ( isset( $_POST['hb_country_iso'] ) ) {
					...
					$country_info['country_iso'] = $_POST['hb_country_iso'];
					if ( ( $_POST['hb_country_iso'] == 'US' ) && isset( $_POST['hb_usa_state_iso'] ) ) {
						$country_info['usa_state_iso'] = $_POST['hb_usa_state_iso'];
					}
					if ( ( $_POST['hb_country_iso'] == 'CA' ) && isset( $_POST['hb_canada_province_iso'] ) ) {
						$country_info['canada_province_iso'] = $_POST['hb_canada_province_iso'];
					}
					$this->hbdb->update_customer_country( $customer['id'], $country_info );
				}
				$currency_to_round = array( 'HUF', 'JPY', 'TWD' );
				if ( in_array( get_option( 'hb_currency' ), $currency_to_round ) || ( get_option( 'hb_price_precision' ) == 'no_decimals' ) ) {
					$amount_to_pay = round( $payment['amount_to_pay'] );
				} else {
					$amount_to_pay = round( $payment['amount_to_pay'], 2 );
				}
				$response = $payment_gateway->process_payment( $resa, $customer, $amount_to_pay );
				if ( $response['payment_token'] ) {
					$this->hbdb->update_later_payment_token( $payment['id'], $response['payment_token'] );
				} else {
					$response['success'] = false;
					$response['error_msg'] = 'Error. Could not find payment token.';
				}
			} else {
				$response['success'] = false;
				$response['error_msg'] = 'Error. Could not find reservation.';
			}
		} else {
			$response['success'] = false;
			$response['error_msg'] = 'Error. Could not process payment.';
		}
		echo( json_encode( $response ) );
		die;
	}

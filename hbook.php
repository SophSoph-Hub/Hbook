...
class HBook {
	...
	public function __construct() {
		...
		require_once plugin_dir_path( __FILE__ ) . 'utils/multi-resa.php';
		$this->multi_resa = new HbMultiResa( $this->hbdb, $this->utils );
		...
		$this->stripe = new HbStripe( $this->hbdb, $this->version, $this->utils );

		require_once plugin_dir_path( __FILE__ ) . 'utils/options-utils.php';
		$this->options_utils = new HbOptionsUtils( $this->hbdb, $this->utils );

		require_once plugin_dir_path( __FILE__ ) . 'accom-post-type/accom-post-type.php';
		$this->accommodation = new HbAccommodation( $this->hbdb, $this->options_utils );
		...
		$this->admin_ajax_actions = new HbAdminAjaxActions( $this->hbdb, $this->utils, $this->options_utils, $this->stripe, $this->resa, $this->multi_resa );
		...
		add_action( 'init', array( $this->hbdb, 'delete_old_resa_logs' ) );
		add_action( 'init', array( $this->utils, 'check_delete_uncompleted_resa' ) );
		...
		add_action( 'init', array( $this->utils, 'reset_resa_invoice_counter' ) );
		add_action( 'init', array( $this->utils, 'update_resa_emails_count' ) );
		add_action( 'init', array( $this->utils, 'update_customer_resa_count' ) );

		add_filter( 'template_include', array( $this->accommodation, 'filter_template_page' ) );
		add_action( 'wp_enqueue_scripts', array( $this->utils, 'frontend_css_files' ) );
		...
		add_filter( 'cron_schedules', array( $this, 'custom_scheduled_frequency' ) );
		add_action( 'hb_ical_synchronized', array( $this, 'ical_update_calendars' ) );
		add_action( 'hb_send_scheduled_emails', array( $this->utils, 'send_scheduled_emails' ) );
		add_action( 'hb_check_resa_payment_delayed', array( $this->utils, 'check_resa_payment_delayed' ) );
		add_action( 'hb_check_plugin', array( $this->utils, 'check_plugin' ) );
		add_action( 'wp_login', array( $this, 'admin_redirect_based_on_role' ), 10, 2 );
		...
		$front_end_ajax_action = array(
			...
			'hb_create_resa',
			'hb_verify_coupon',
			'hb_stripe_declined_payment',
			'hb_process_later_payment',
		);
		...
		$admin_ajax_action = array(
			...
			'hb_resa_charge',
			'hb_resa_refund',
			'hb_refund_resa_payment',
			'hb_update_resa_dates',
			'hb_send_email_customer',
			'hb_save_resa_sorting',
			'hb_save_customers_sorting',
			'hb_fetch_email_logs',
		);
		...
		add_filter( 'block_categories_all', array( $this->blocks, 'add_blocks_category' ) );
	...
	public function install_plugin() {
		...
		if ( ! $installed_version ) {
			...
			set_transient( 'hbook-activation-notice', true, 5 );
		} else if ( $installed_version != $this->version ) {
			$hbdb_creation->alter_data( $installed_version );
		}
		if ( ! $installed_version || $installed_version != $this->version ) {
			$this->options_utils->init_options( $installed_version );
	...
	public function plugin_activated() {
		...
		wp_schedule_event( time() + 30, 'hb_ical_custom_frequency', 'hb_ical_synchronized' );
		wp_schedule_event( time() + 30, 'hourly', 'hb_send_scheduled_emails' );
		if ( $this->utils->payment_gateways_have_delayed_payment() ) {
			wp_schedule_event( time() + 30, 'hourly', 'hb_check_resa_payment_delayed' );
		}
		wp_schedule_event( time() + 30, 'daily', 'hb_check_plugin' );
		$roles_caps = $this->utils->get_hbook_roles_caps();
		foreach ( $roles_caps as $role => $info ) {
			$caps = array();
			$caps_list = apply_filters( $role . '_capabilities', $info['caps'] );
			foreach ( $caps_list as $key => $cap ) {
				$caps[ $cap ] = true;
			}
			add_role(
				$role,
	...
	public function plugin_deactivated() {
		...
		wp_clear_scheduled_hook( 'hb_send_scheduled_emails' );
		wp_clear_scheduled_hook( 'hb_check_resa_payment_delayed' );
	...
	public function custom_scheduled_frequency( $schedules ) {
		$ical_frequency = get_option( 'hb_ical_frequency', 3600 );
		if ( ! $ical_frequency ) {
			$ical_frequency = 3600;
		}
		$schedules['hb_ical_custom_frequency'] = array(
			'interval' => $ical_frequency,
			'display' => esc_html__( 'HBook iCal custom frequency', 'hbook-admin' ) . ' (' . $ical_frequency . 's)'
		);
		$custom_check_resa_payment_delayed_frequency = get_option( 'hb_check_resa_payment_delayed_frequency' );
		if ( $custom_check_resa_payment_delayed_frequency && ( $custom_check_resa_payment_delayed_frequency != 'hourly' ) ) {
			$custom_check_resa_payment_delayed_frequency = intval( $custom_check_resa_payment_delayed_frequency );
			if ( ! $custom_check_resa_payment_delayed_frequency ) {
				$custom_check_resa_payment_delayed_frequency = 3600;
			}
			$schedules['hb_check_resa_payment_delayed_custom_frequency'] = array(
				'interval' => $custom_check_resa_payment_delayed_frequency,
				'display' => esc_html__( 'HBook check delayed payments custom frequency', 'hbook-admin' ) . ' (' . $custom_check_resa_payment_delayed_frequency . 's)'
			);
		}
		return $schedules;
	}
	...
	function admin_redirect_based_on_role( $user_login, $user ) {
		if ( ( in_array( 'hb_resa_manager', $user->roles ) ) || ( in_array( 'hb_resa_reader', $user->roles ) ) ) {
			exit( wp_redirect( admin_url( 'admin.php?page=hb_reservations' ) ) );
		}
	}

	public function hbook_activation_notice() {
		if ( get_transient( 'hbook-activation-notice' ) ) {
			delete_transient( 'hbook-activation-notice' );
		?>
		...
		<div class="updated">
			<p>
				...
				if ( strpos( $doc_msg, '%s' ) ) {
					$doc_msg = str_replace( '%s', '<a target="_blank" href="' . $doc_url . '">'  . $doc_word . '</a>', $doc_msg );
				} else {
					$doc_msg .= ' (<a target="_blank" href="' . $doc_url. '">'  . $doc_word . '</a>)';
				}
				...
				if ( strpos( $knowledge_msg, '%s' ) ) {
					$knowledge_msg = str_replace( '%s', '<a target="_blank" href="' . $knowledge_url . '">' . $knowledge_word . '</a>', $knowledge_msg );
				} else {
					$knowledge_msg .= ' (<a target="_blank" href="' . $knowledge_url . '">' . $knowledge_word . '</a>)';
				}
				?>
				<?php echo( wp_kses_post( $thanks_msg ) ); ?>
				<br/>
				<?php echo( wp_kses_post( $doc_msg ) ); ?>
				<br/>
				<?php
				echo( wp_kses_post( $knowledge_msg ) );
				$admin_lang = '';
				if ( $locale == 'fr_FR' ) {
					$admin_lang = 'French';
	...
	public function init_plugin() {
		if (
			...
			current_user_can( 'manage_hb_licence' )
		) {
		...
		if ( ! wp_next_scheduled( 'hb_send_scheduled_emails' ) ) {
			wp_schedule_event( time() + 30, 'hourly', 'hb_send_scheduled_emails' );
		}
		if ( ! wp_next_scheduled( 'hb_check_plugin' ) ) {
			wp_schedule_event( time() + 30, 'daily', 'hb_check_plugin' );
		}
		if (
			$this->utils->payment_gateways_have_delayed_payment() &&
			(
				( get_option( 'hb_reset_check_resa_payment_delayed' ) == 'yes' ) ||
				! wp_next_scheduled( 'hb_check_resa_payment_delayed' )
			)
		) {
			...
			$custom_check_resa_payment_delayed_frequency = get_option( 'hb_check_resa_payment_delayed_frequency' );
			if ( $custom_check_resa_payment_delayed_frequency && ( $custom_check_resa_payment_delayed_frequency != 'hourly' ) ) {
				wp_schedule_event( time() + 30, 'hb_check_resa_payment_delayed_custom_frequency', 'hb_check_resa_payment_delayed' );
			} else {
				wp_schedule_event( time() + 30, 'hourly', 'hb_check_resa_payment_delayed' );
			}
		}
	...
	public function enqueue_wp_admin_style() {
		...
		$this->utils->hb_enqueue_script( 'hb-global', '/admin-pages/js/hb-global.js' );
		...
		if ( $post_type == 'hb_accommodation' ) {
			...
			$this->utils->hb_script_var( 'hb-accom-script', 'hb_accom_post_text', $hb_accom_post_text );
		}
	}
	...
	public function enqueue_scripts( $hook ) {
		...
		if ( $page_name == 'fees' || $page_name == 'options' || $page_name == 'rates' || $page_name == 'reservations' ) {
			...
			$this->utils->hb_script_var( 'hb-price-utils', 'hb_currency_pos', get_option( 'hb_currency_position' ) );
			$this->utils->hb_script_var( 'hb-price-utils', 'hb_currency', get_option( 'hb_currency' ) );
		}
	...
	public function create_plugin_admin_menu() {
		...
		foreach ( $hbook_pages as $p ) {
			$page_cap = 'manage_' . $p['id'];
			$page = add_submenu_page( 'hb_menu', $p['name'], $p['name'], $page_cap, $p['id'], array( $this, 'display_admin_page' ) );
			add_action( 'admin_print_styles-' . $page, array( $this, 'enqueue_scripts' ) );
		...
		$page = add_submenu_page( '', 'Logs', 'Logs', 'manage_options', 'hb_logs', array( $this, 'display_admin_page' ) );
	}

	public function display_admin_page() {
		$hb_page = $_GET['page'];
		$page_id = str_replace( 'hb_', '', $hb_page );
		$page_cap = 'manage_' . $hb_page;
		if ( current_user_can( $page_cap ) || ( $page_id == 'reservations' && current_user_can( 'read_resa' ) ) ) {

...
class HbOptionsUtils {
	...
	public function get_payment_settings() {
		return array(
			...
			'payment_link' => array(
				'label' => esc_html__( 'Payment link settings', 'hbook-admin' ),
				'options' => array(
					'hb_payment_link_page_id' => array(
						'label' => esc_html__( 'Page where customers following a payment link will be redirected', 'hbook-admin' ),
						'caption' => esc_html__( 'The payment link page should contain a "Payment link form" block or a [hb_payment_link_form] shortcode.', 'hbook-admin' ),
						'type' => 'select',
						'choice' => array(),
					),
					'hb_payment_link_appearance' => array(
						'label' => esc_html__( 'Payment link appearance', 'hbook-admin' ),
						'type' => 'radio',
						'choice' => array(
							'button' => esc_html__( 'Button', 'hbook-admin' ),
			...
			'offline_payment' => array(
				'label' => esc_html__( 'Offline payment methods', 'hbook-admin' ),
				'options' => array(
					'hb_offline_payment_methods' => array(
						'label' => esc_html__( 'List of offline payment methods separated with comma', 'hbook-admin' ),
						'caption' => esc_html__( 'Used in the "Add a payment" form of the admin Reservation page.', 'hbook-admin' ),
						'type' => 'text',
						'default' => esc_html__( 'Cash, Wire transfer', 'hbook-admin' ),
					),
				)
			),
	...
	public function get_appearance_settings() {
		return array(
			...
			'tables_appearance' => array(
				'label' => esc_html__( 'Tables appearance', 'hbook-admin' ),
				'options' => array(
					'hb_tables_style' => array(
						'label' => esc_html__( 'Tables style', 'hbook-admin' ),
						'type' => 'radio',
						'choice' => array(
							'theme' => esc_html__( 'Use theme styles', 'hbook-admin' ),
			...
			'misc' => array(
				...
				'options' => array(
					...
					'hb_admin_language' => array(
						...
						'caption' => str_replace(
							'%s',
							'<a target="_blank" href="https://maestrel-resource-center.refined.site/space/KB/34734439/">' .
	...
	public function init_options( $installed_version ) {
		...
		if ( get_option( 'hb_add_stripe_logs' ) === false ) {
			update_option( 'hb_add_stripe_logs', 'no' );
		}
	}
		...
		return array(
			'non_standard' => array(
				'options' => array(
					...
					'hb_stripe_logs' => array(),
					'hb_add_stripe_logs' => array(),
				)
			)
		);
	}

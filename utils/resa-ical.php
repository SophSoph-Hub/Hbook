...
class HbResaIcal {
	...
	public function export_ical() {
		...
		if ( $accom_post ) {
			...
			foreach ( $default_headers as $default_header_key => $default_header_value ) {
				header( $default_header_key . ": " . $default_header_value );
			}
			$additional_headers = apply_filters( 'hb_ical_export_additional_headers', array() );
			$accepted_headers = array( 'X-LWS-Cache' );
			foreach ( $additional_headers as $additional_header_key => $additional_header_value ) {
				if ( in_array( $additional_header_key, $accepted_headers ) ) {
					header( $additional_header_key . ": " . $additional_header_value );
				}
			}
	...
	public function create_ical( $accom_id, $accom_num, $agenda, $future_only, $reservations_only, $global_calendar ) {
		...
		foreach ( $reservations as $reservation ) {
			...
			if ( false !== strpos( $reservation['uid'] ,'Booking-Ref-' ) ) {
				$channel_booking_ref = substr( $reservation['uid'], 12 );
				$custom_fields['X-Channel-Booking-Ref'] = $channel_booking_ref;
			}
...
<?php
	...
	private function process_ical_file( $ical_data, $calendar_name, $accom_id, $accom_num, $db_calendar_id, $synchro_id, $synchro_url ) {
		...
		$ics_calendar_arrays = $this->ics_to_array( $ical_data );
		$count = 0;
		$ignored_events = 0;
		$results = array();
		$non_reliable_uid_calendar = false;
		$organizer_as_uid = false;
		$booking_ref_as_uid = false;
		$airbnb_calendar = false;
		$invalid_signature = false;
		$uid_list = array();
		foreach ( $ics_calendar_arrays as $ics_calendar_array ) {
			$calendar_array = array_change_key_case( $ics_calendar_array, CASE_UPPER );
			$admin_comment = '';
			$origin_url = '';
			...
			if ( isset ( $calendar_array['BEGIN'] ) && ( $calendar_array['BEGIN'] == 'VCALENDAR' ) ) {
				...
				$unreliable_prodids = array(
					'avantio', 'ferienhausmiete', 'GitesBV Inc', 'Listing-', 'Lodgify', 'Orchestra', 'OYO', 'rianjs', 'Travanto', 'TripAdvisor', 'tuivillas', 'weebnb'
				);
				foreach ( $unreliable_prodids as $prodid ) {
					if ( false !== strpos( $calendar_array['PRODID'], $prodid ) ) {
				...
				if ( false !== strpos( $calendar_array['PRODID'] ,'Campsites.co.uk' ) ) {
					$booking_ref_as_uid = true;
				}
				...
				if ( $db_calendar_id ) {
					if ( $calendar_id != $db_calendar_id ) {
						...
						||
							( false !== strpos( $calendar_id, 'Sabre//Sabre' ) && false !== strpos( $db_calendar_id, 'Sabre//Sabre' ) )
						||
				...
				if ( false === strpos( $calendar_array['SUMMARY'],'PENDING' ) && isset( $calendar_array['DTEND'] ) && isset( $calendar_array['DTSTART'] ) ) {
					...
					if ( strtotime( $dtend ) >= strtotime ( current_time( 'Y-m-d' ) ) ) {
						...
						if ( strtotime( $dtend ) > strtotime( $dtstart ) ) {
							...
							$existing_resa = '';
							$uid = '';
							if ( isset( $calendar_array['UID'] ) && ! $non_reliable_uid_calendar && ! $organizer_as_uid && ! $booking_ref_as_uid ) {
								$uid = $calendar_array['UID'];
							} else if ( $organizer_as_uid ) {
								$uid = $dtstart . '-' . $calendar_array['ORGANIZER'];
							} else if ( $booking_ref_as_uid ) {
								$uid = 'Booking-Ref-' . $calendar_array['X-BOOKING-REF'];
							...
							if ( $existing_resa ) {
								...
								if ( ! $is_available ) {
									...
									$description_insensitive_case = array();
									if ( isset( $calendar_array['SUMMARY'] ) ) {
										if ( ( $calendar_array['SUMMARY'] != 'Reserved' ) && ( ! $organizer_as_uid ) ) {
											$admin_comment .= $calendar_array['SUMMARY'] . "\n";
										}
									}
									$customer_ical_non_standard_properties = array(
										'email' => array( 'X-EMAIL' ),
										'last_name' => array( 'X-NAME' ),
										'phone' => array( 'X-PHONE', 'X-TELEPHONE' ),
										'address_1' => array( 'X-ADDRESS' ),
										'zip_code' => array( 'X-POSTCODE'),
									);

									foreach ( $customer_ical_non_standard_properties as $hb_customer_data => $ical_property_names ) {
										foreach ( $ical_property_names  as $property_name ) {
											if ( isset( $calendar_array[ $property_name ] ) ) {
												$customer_properties[ $hb_customer_data ] = $calendar_array[ $property_name ];
											}
										}
									}

									if ( isset( $customer_properties['email'] ) ) {
										$email = $customer_properties['email'];
									}

									if ( isset( $calendar_array['DESCRIPTION'] ) ) {
										$description_insensitive_case = array_change_key_case( $calendar_array['DESCRIPTION'], CASE_UPPER );
										if ( ! $email ) {
									...
									$other_ical_non_standard_properties = array();
									$additional_ical_non_standard_properties = apply_filters( 'hb_ical_additional_non_standard_properties', $other_ical_non_standard_properties );
									foreach ( $additional_ical_non_standard_properties as $non_standard_property ) {
										if ( isset( $calendar_array[ strtoupper( $non_standard_property ) ] ) ){
											$key = $non_standard_property;
											if ( 0 == strpos( strtoupper( $key ), 'X-' ) ) {
												$key = substr( $key, 2 );
											}
											$key = ucfirst( strtolower( $key ) );
											$value = ucfirst( strtolower( $calendar_array[ strtoupper( $non_standard_property ) ] ) );
											$admin_comment .= trim( stripslashes( strip_tags( preg_replace( '~[\n]+~', '', $key ) ) ) ) . ': ' . trim( stripslashes( strip_tags( preg_replace( '~[\n]+~', '', $value ) ) ) ) . "\n";
										}
									}

									if ( isset( $calendar_array['X-BOOKINGURL'] ) ) {
										$origin_url = $calendar_array['X-BOOKINGURL'];
									}

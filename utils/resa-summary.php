...
class HbResaSummary {
	...
	private $fixed_coupon_amount_left;
	private $is_admin;
	...
	public function get_summary( $resa, $parent_resa = false, $customer_info = false, $payment_type = false ) {
		...
		if ( isset( $_GET['payment_id'] ) || ( $payment_type == 'paid_later_payment' ) || ( $payment_type == 'method_updated_later_payment' ) ) {
			$is_later_payment = true;
		} else {
		...
		if ( $payment_type == 'paid_later_payment' ) {
			$thanks_message = str_replace( '%customer_email', $customer_info['email'], $this->strings['later_payment_thanks_message'] );
			$output .= '<p class="hb-later-payment-thank-you-summary-top-msg">' . $thanks_message . '</p>';
		} else if ( $payment_type == 'method_updated_later_payment' ) {
			...
			$output .= '<p class="hb-later-payment-thank-you-summary-top-js-msg">&nbsp;</p>';
		...
		if ( $is_later_payment && $this->strings['summary_resa_details_subtitle'] ) {
			$output .= '<h4 class="hb-subtitle hb-resa-summary-details-subtitle">';
			$output .= $this->strings['summary_resa_details_subtitle'];
			$output .= '</h4>';
		}
		$output .= '<div class="hb-resa-summary-details">';
		...
		foreach ( $resa as $i => $resa_info ) {
			...
			if ( $is_later_payment && ! $is_multiple_resa && $this->strings['summary_price_breakdown_subtitle'] ) {
				$output .= '<h4 class="hb-subtitle hb-resa-summary-price-breakdown-subtitle">';
				$output .= $this->strings['summary_price_breakdown_subtitle'];
				$output .= '</h4>';
			}
		...
		if ( $this->is_admin || ( get_option( 'hb_display_price' ) == 'yes' ) ) {
			...
			if ( $is_later_payment ) {
				if ( $parent_resa ) {
					$payments_history_table = $this->utils->get_payments_history_table( $parent_resa, true, true );
				} else {
					$payments_history_table = $this->utils->get_payments_history_table( $resa[0], false, true );
				}
				if ( $payments_history_table ) {
					if ( $this->strings['summary_payments_history_subtitle'] ) {
						$output .= '<h4 class="hb-subtitle hb-resa-summary-payments-history-subtitle">';
						$output .= $this->strings['summary_payments_history_subtitle'];
						$output .= '</h4>';
					} else {
						$output .= '<br/>';
					}
					$output .= '<div class="hb-resa-summary-payments-history-table-wrapper">';
					$output .= $payments_history_table;
					$output .= '</div>';
				}
			}
		...
		if ( $is_later_payment || ( $payment_type === false ) ) {

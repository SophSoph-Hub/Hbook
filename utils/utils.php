...
class HbUtils {
	...
	public function __construct( $hbdb, $plugin_version ) {
		...
		require_once $this->plugin_directory . '/utils/resa.php';
		$this->resa = new HbResa( $hbdb, $this );
	...
	public function get_page_padding_top() {
		$page_padding_top = trim( get_option( 'hb_page_padding_top' ) );
		if ( $page_padding_top !== '0' ) {
			$page_padding_top = intval( $page_padding_top );
			if ( ! $page_padding_top ) {
	...
	public function load_front_end_script( $script ) {
		...
		switch ( $script ) {
			...
			case 'selectize' :
				...
				break;
			case 'payment-link-form' :
				$this->hb_enqueue_script( 'hb-payment-link-form-script', '/front-end/js/payment-link-form.js' );
				break;
			case 'booking-form' :
	...
	public function send_not_automatic_email( $resa_id, $resa_is_parent ) {
		...
		$payment_link_types = $this->get_payment_link_types();
		foreach ( $payment_link_types as $payment_link_type => $payment_link_type_label ) {
			if ( strpos( $_POST['email_message'], '[payment_link_' . $payment_link_type ) !== false ) {
				$is_html_email = true;
			}
		}
		if (
			( strpos( $_POST['email_message'], '[resa_invoice_table]' ) !== false ) ||
			( strpos( $_POST['email_message'], '[resa_payments_history_table]' ) !== false )
		) {
			$is_html_email = true;
		}
	...
	private function filter_email_templates( $email_templates, $resa ) {
		...
		} else if ( $this->resa_total_paid( $resa ) == 0 ) {
			$resa_payment_status = 'unpaid';
		} else if ( get_option( 'hb_security_bond_online_payment' ) == 'yes' ) {
			if ( $this->resa_total_paid( $resa ) < $resa['price'] ) {
				$resa_payment_status = 'not_fully_paid';
			} else if (
				( $this->resa_total_paid( $resa ) < ( $resa['price'] + floatval( get_option( 'hb_security_bond_amount' ) ) ) ) &&
	...
	public function replace_resa_vars_with_value( $resa_id, $is_html, $text, $resa_is_parent = false ) {
		...
		if ( $is_html ) {
			...
			$text = str_replace( '[resa_remaining_balance]', $this->price_with_symbol( $resa['price'] - $this->resa_total_paid( $resa ) ), $text );
			...
			$text = str_replace( '[resa_remaining_balance_including_bond]', $this->price_with_symbol( $resa['price'] + $bond - $this->resa_total_paid( $resa ) ), $text );
			...
			$text = str_replace( '[resa_remaining_balance]', $this->price_with_currency_letters( $resa['price'] - $this->resa_total_paid( $resa ) ), $text );
			...
			$text = str_replace( '[resa_remaining_balance_including_bond]', $this->price_with_currency_letters( $resa['price'] + $bond - $this->resa_total_paid( $resa ) ), $text );
		...
		foreach ( $payment_link_types as $payment_link_type => $payment_link_type_label ) {
			if (
				(
					( $payment_link_type != 'custom_amount' ) &&
					( strpos( $text, '[payment_link_' . $payment_link_type . ']' ) !== false )
				) ||
				(
					( $payment_link_type == 'custom_amount' ) &&
					preg_match( '/\[payment_link_custom_amount-(\d+\.?\d*)\]/', $text, $payment_link_custom_amount_matches )
				)
			) {
				if ( $payment_link_type == 'custom_amount' ) {
					$payment_link_type .= '-';
					$payment_link_type .= $payment_link_custom_amount_matches[1];
				}
				if ( ! $is_html ) {
					$text = str_replace( '[payment_link_' . $payment_link_type . ']', '', $text );
				} else {
					$payment_link_page_id = get_option( 'hb_payment_link_page_id' );
					$payment_link_error = false;
					if ( ! $payment_link_page_id ) {
						$payment_link = esc_html( 'Could not generate a payment link because no Payment link page has been set up.', 'hbook-admin' );
						$payment_link_error = true;
					} else {
						$link_alphanum = $this->hbdb->add_later_payment( $resa['id'], $resa_is_parent, $payment_link_type );
						if ( $link_alphanum ) {
							$payment_link = $this->get_translated_post_url_by_locale( $payment_link_page_id, $this->email_doc_locale );
							if ( strpos( $payment_link, '?' ) ) {
								$payment_link .= '&';
							} else {
								$payment_link .= '?';
							}
							$payment_link .= 'payment_id=' . $link_alphanum;
						} else {
							$payment_link = 'Unexpected error while generating a payment link.';
							$payment_link .= $this->hbdb->last_error();
							$payment_link_error = true;
						}
					}
					if ( ! $payment_link_error ) {
						$payment_link_style = '';
						if ( get_option( 'hb_payment_link_appearance' ) == 'button' ) {
							$buttons_css_rules = $this->buttons_css_rules();
							$buttons_css_options = json_decode( get_option( 'hb_buttons_css_options' ), true );
							$payment_link_button_style = '';
							foreach ( $buttons_css_rules as $rule_id => $rule ) {
								if ( $rule_id != 'bg_hover' ) {
									...
									foreach ( $rule['property'] as $rule_property ) {
										$payment_link_button_style .= $rule_property . ': ' . $rule_value;
										if ( $rule['type'] == 'number' ) {
											$payment_link_button_style .= 'px';
										}
										$payment_link_button_style .= '; ';
									}
								}
							}
							$payment_link_button_style .= 'text-decoration: none; display: inline-block;';
							$payment_link_button_style = apply_filters( 'hb_payment_link_button_style', $payment_link_button_style );
							if ( $payment_link_type == 'no_payments' ) {
								$payment_link_button_txt = $this->get_string( 'later_payment_update_button_txt', $this->email_doc_locale );
							} else {
								$payment_link_button_txt = $this->get_string( 'later_payment_pay_button_pay_txt', $this->email_doc_locale );
							}
							$payment_link = '<p class="hb-payment-link-button-wrapper"><a href="' . $payment_link . '" class="hb-payment-link-button" style="' . esc_attr( $payment_link_button_style ). '">' . esc_html( $payment_link_button_txt ) . '</a></p>';
							$payment_link = apply_filters( 'hb_payment_link_button_markup', $payment_link );
						} else {
							$payment_link = '<a href="' . $payment_link . '" class="hb-payment-link-text">' . $payment_link . '</a>';
							$payment_link = apply_filters( 'hb_payment_link_text_markup', $payment_link );
						}
					}
					$text = str_replace( '[payment_link_' . $payment_link_type . ']', $payment_link, $text );
				}
			}
		}
		...
		if ( strpos( $text, '[resa_payments_history_table]' ) !== false ) {
			$payments_history_table = $this->get_payments_history_table( $resa, $resa_is_parent );
			$text = str_replace( '[resa_payments_history_table]', $payments_history_table, $text );
		}

		return $text;
	}

	public function get_payment_link_types() {
		if ( get_option( 'hb_security_bond' ) == 'yes' ) {
			$payment_link_types = array(
				'total_price' => esc_html_x( 'Total price', 'payment link', 'hbook-admin' ),
				'total_price_including_bond' => esc_html__( 'Total price including bond', 'hbook-admin' ),
				'deposit' => esc_html__( 'Deposit', 'hbook-admin' ),
				'deposit_including_bond' => esc_html__( 'Deposit including bond', 'hbook-admin' ),
				'remaining_balance' => esc_html__( 'Remaining balance', 'hbook-admin' ),
				'remaining_balance_including_bond' => esc_html__( 'Remaining balance including bond', 'hbook-admin' ),
				'bond' => esc_html__( 'Bond', 'hbook-admin' ),
				'custom_amount' => esc_html__( 'Custom amount', 'hbook-admin' ),
			);
		} else {
			$payment_link_types =  array(
				'total_price' => esc_html_x( 'Total price', 'payment link', 'hbook-admin' ),
		...
		if ( ( get_option( 'hb_stripe_active' ) == 'yes' ) && ( get_option( 'hb_store_credit_card' ) == 'yes' ) ) {
			$payment_link_types['no_payments'] = esc_html__( 'No payments (store payment method only)', 'hbook-admin' );
		}
		return $payment_link_types;
	}

	public function get_payments_history_table( $resa, $resa_is_parent, $summary_table = false ) {
		$payments_logs = $this->prepare_resa_payments_logs( $resa, false );
		if ( ! $payments_logs ) {
			return '';
		} else {
			if ( $summary_table ) {
				$locale = $this->email_doc_locale;
			} else {
				$locale = '';
			}
			$table = '<table class="invoice-table payments-history-table">';
			$table .= '<tr>';
			$table .= '<th class="payment-date-head">';
			$table .= $this->get_string( 'table_payments_history_head_date', $locale );
			$table .= '</th>';
			$table .= '<th class="payment-method-head">';
			$table .= $this->get_string( 'table_payments_history_head_method', $locale );
			$table .= '</th>';
			$table .= '<th class="amount-head">';
			$table .= $this->get_string( 'table_payments_history_head_amount', $locale );
			$table .= '</th>';
			$table .= '</tr>';
			$total_amount_paid = 0;
			$nb_payments = 0;
			foreach ( $payments_logs as $payment ) {
				if ( ! isset( $payment['failed'] ) ) {
					$nb_payments++;
					$table .= '<tr>';
					$table .= '<td>';
					if ( $payment['received_on'] ) {
						$table .= $this->format_date( $payment['received_on'] );
					}
					$table .= '</td>';
					$table .= '<td>' . $payment['payment_method'] . '</td>';
					$table .= '<td class="amount">' . $this->price_with_symbol( $payment['amount'] ) . '</td>';
					$total_amount_paid += $payment['amount'];
				}
			}
			if ( $nb_payments > 1 ) {
				$table .= '<tr class="total">';
				$total_text = $this->get_string( 'table_payments_history_total_paid', $locale );
				if ( $total_text ) {
					...
					$table .= '<td colspan="2" class="empty"></td>';
				}
				$table .= '<td class="amount">' . $this->price_with_symbol( $total_amount_paid ) . '</td>';
				$table .= '</tr>';
			}
			$table .= '</table>';
		}
		if ( ! $summary_table ) {
			$style = $this->get_table_style();
			if ( $style ) {
				$style = apply_filters( 'hb_payments_history_table_style', $style, $resa );
			}
			$table .= $style;
		}
		$table = apply_filters( 'hb_payments_history_markup', $table, $resa );
		return $table;
	}

	private function get_table_style() {
		static $styled = false;
		if ( $styled ) {
			return '';
		}
		...
		$style .= '.invoice-table td.empty { border: none; width: 33%; }';
		$style .= '.payments-history-table td.empty { border: none; }';
		$style .= '.payments-history-table .total-text { border-right: none; }';
		$style .= '.payments-history-table .amount { border-left: none; }';
		$style .= '.invoice-table td.multi-accom-empty { border: none; width: 31%; }';
		$style .= '.invoice-table td.multi-accom-indent { background: #eee; border-bottom: none; border-top: none; padding: 0; width: 2%; }';
		$style .= '.invoice-table .multi-accom-separator td { background: #ddd; height: 1px; padding: 0; }';
		$style .= '.invoice-table td.subpart-title { background: #eee; font-size: 17px; font-weight: bold; height: 46px; text-align: center; }';
		$style .= '.invoice-table td.subsubpart-title { background: #eee; font-weight: bold; }';
		$style .= '.payments-history-table .total-text, .subtotal .amount, .total .amount, .due .amount { position: relative; }';
		$style .= '.payments-history-table .total-text:before, .subtotal .amount:before, .total .amount:before, .due .amount:before { background: #ddd; content: ""; display: block; left: 0; position: absolute; right: 0; top: 0; }';
		$style .= '.subtotal .amount:before { height: 3px; }';
		$style .= '.payments-history-table .total-text:before, .total .amount:before, .due .amount:before { height: 7px; }';
		$style .= '.total td, .due td { text-transform: uppercase; }';
		$style .= '.fee-included, .fee-included + .amount { font-size: 11px; }';
		$style .= '.paid .paid-text, .paid .amount, .due .due-text, .due .amount { background: #f7f7f7; }';
		$style .= '.desc-head { width: 75%; }';
		$style .= '.payment-date-head, payment-method-head { width: 37.5%; }';
		$style .= '.amount-head { width: 25%; }';
		$style .= '.amount, .fee-final, .fee-included, .subtotal-text, .coupon-text, .discount-text, .total-text, .bond-text, .total-with-bond-text, .paid-text, .due-text { text-align: right; }';
		$style .= '.bond .bond-text, .bond .amount { border-top: 3px solid #ddd; }';
		$style .= '.subtotal-text, .total-text, .subtotal .amount, .total .amount, .paid, .due { font-weight: bold }';
		$style .= '</style>';
		return $style;
	}
	...
	private function get_invoice_table( $resa, $resa_is_parent ) {
		...
		foreach ( $children_resa as $resa ) {
			if ( ( $resa['accom_id'] != 'parent_resa' ) && ( $resa['status'] == 'cancelled' ) ) {
				continue;
			}
			...
			if ( $resa['accom_id'] != 'parent_resa' ) {
				...
				if ( $resa_is_parent ) {
					...
					$table .= '<td colspan="3">';
					$table .= $accom_name;
					if ( $resa['accom_num'] && $this->show_accom_num_for_invoice() ) {
						$table .= ' (';
		...
		if ( get_option( 'hb_security_bond_online_payment' ) == 'yes' ) {
			...
			if ( $resa['check_out'] >= $today ) {
				$security_bond = get_option( 'hb_security_bond_amount' );
			}
		}

		if ( $this->resa_total_paid( $resa ) > 0 ) {
			if ( $security_bond ) {
				$table .= '<tr class="bond">';
				$table .= '<td colspan="2" class="empty">';
				$table .= '</td>';
				$table .= '<td class="bond-text">';
				$table .= $this->get_string( 'table_invoice_bond', $this->email_doc_locale );
				$table .= '</td>';
				$table .= '<td class="amount">';
				$table .= $this->price_with_symbol( $security_bond );
				$table .= '</td>';
				$table .= '</tr>';

				$table .= '<tr class="total-with-bond">';
				$table .= '<td colspan="2" class="empty">';
				$table .= '</td>';
				$table .= '<td class="total-with-bond-text">';
				$table .= $this->get_string( 'table_invoice_total_with_bond', $this->email_doc_locale );
				$table .= '</td>';
				$table .= '<td class="amount">';
				$table .= $this->price_with_symbol( $total + $security_bond );
				$table .= '</td>';
				$table .= '</tr>';
			}
			...
			$table .= '<td colspan="2" class="empty">';
			$table .= '</td>';
			$table .= '<td class="paid-text">';
			$table .= $this->get_string( 'table_invoice_paid', $this->email_doc_locale );
			$table .= '</td>';
			$table .= '<td class="amount">';
			$table .= $this->price_with_symbol( $this->resa_total_paid( $resa ) );
			$table .= '</td>';
			$table .= '</tr>';
			...
			$table .= '<td colspan="2" class="empty">';
			$table .= '</td>';
			$table .= '<td class="due-text">';
			$table .= $this->get_string( 'table_invoice_due', $this->email_doc_locale );
			$table .= '</td>';
			$table .= '<td class="amount">';
			$table .= $this->price_with_symbol( $security_bond + $resa['price'] - $this->resa_total_paid( $resa ) );
			$table .= '</td>';
			$table .= '</tr>';
		}
	...
	private function show_accom_num_for_invoice() {
		$show_accom_num = false;
	...
	public function get_ical_email_document_available_vars( $vars_for = '' ) {
		$vars = array(
			'reservation' => array(
				'[resa_id]',
				'[resa_ids]',
		...
		if ( $this->payment_gateways_support_later_payment() && ( $vars_for != 'ical' ) ) {
			$payment_link_types = $this->get_payment_link_types();
			foreach ( $payment_link_types as $payment_link_type => $payment_link_type_label ) {
				if ( $payment_link_type != 'custom_amount' ) {
					$vars['payment'][] = '[payment_link_' . $payment_link_type . ']';
				} else {
					$vars['payment'][] = '[payment_link_custom_amount-xx.xx]';
				}
			}
		}
		...
		foreach ( $resa_additional_fields as $field ) {
			$vars['reservation'][] = '[resa_' . $field['id'] . ']';
		}

		$vars['customer'] = array( '[customer_id]' );
		$customer_fields = $this->hbdb->get_customer_form_fields();
		foreach ( $customer_fields as $field ) {
			if ( $field['id'] == 'country_iso' ) {
				$vars['customer'][] = '[customer_country]';
			} else {
				$vars['customer'][] = '[customer_' . $field['id'] . ']';
			}
		}
	...
	public function check_resa_payment_delayed() {
		...
		foreach ( $resas as $resa ) {
			$payments_logs = json_decode( $resa['payments_logs'], true );
			$updated_payments_logs = array();
			$resa_payment_updated = false;
			$resa_has_delayed_payment = false;
			foreach ( $payments_logs as $payment ) {
				if ( isset( $payment['delayed'] ) ) {
					$payment_gateway = $this->get_payment_gateway( $payment['payment_method'] );
					if ( $payment_gateway ) {
						$payment_info = $payment_gateway->check_resa_payment_delayed_status( $payment['gateway_info'] );
						if ( ( $payment_info['status'] == 'succeeded' ) || ( $payment_info['status'] == 'failed' ) ) {
							$resa_payment_updated = true;
							unset( $payment['delayed'] );
							if ( ( $payment_info['status'] == 'succeeded' ) && ( $payment['payment_method'] == 'Stripe' ) ) {
								$payment['gateway_info'] = $payment_info['info'];
							}
							if ( $payment_info['status'] == 'failed' ) {
								$payment['failed'] = 'yes';
							}
						} else if ( $payment_info['status'] == 'delayed' ) {
							$resa_has_delayed_payment = true;
						}
					}
				}
				$updated_payments_logs[] = $payment;
			}
			if ( $resa_payment_updated ) {
				$this->hbdb->update_resa_payment_delayed( $resa, $updated_payments_logs, $resa_has_delayed_payment );
			}
		}
	}
	...
	private function resa_options_generic( $chosen_options, $is_markup, $is_admin ) {
		...
		if ( ! $this->extra_list_loaded ) {
			$this->load_extra_list();
		}
		$options_text = '';
		$bold_begin = '';
		$bold_end = '';
	...
	private function load_extra_list() {
		$tmp_options = array();
		...
		$this->extra_list_loaded = true;
	}

	public function resa_total_paid( $resa ) {
		$total_paid = 0;
		if ( $resa['payments_logs'] ) {
			$payments_logs = json_decode( $resa['payments_logs'], true );
			foreach ( $payments_logs as $payment_log ) {
				if ( ! isset( $payment_log['failed'] ) ) {
					$total_paid += $payment_log['amount'];
				}
			}
		}
		if ( strpos( $resa['payment_info'], 'legacy_stripe_charges' ) ) {
			$payment_info = json_decode( $resa['payment_info'], true );
			$legacy_stripe_charges_amount = 0;
			foreach ( $payment_info['legacy_stripe_charges'] as $legacy_charge ) {
				$legacy_stripe_charges_amount += $legacy_charge['amount'];
			}
			$total_paid += $resa['paid'] - $legacy_stripe_charges_amount;
		} else {
			$total_paid += $resa['paid'];
		}
		return $total_paid;
	}

	public function prepare_resa_payments_logs( $resa, $desc_order = true ) {
		$initial_payments = array();
		if ( $resa['paid'] != 0 ) {
			if ( $resa['payment_gateway'] == 'Stripe' ) {
				$stripe_charges_amount = 0;
				if ( $resa['payment_info'] ) {
					$resa_payment_info = json_decode( $resa['payment_info'], true );
					if (
						$resa_payment_info &&
						isset( $resa_payment_info['stripe_charges'] ) &&
						is_array( $resa_payment_info['stripe_charges'] )
					) {
						foreach ( $resa_payment_info['stripe_charges'] as $stripe_charge ) {
							if ( isset( $stripe_charge['amount'] ) ) {
								$stripe_charges_amount += $stripe_charge['amount'];
								$initial_payments[] = array(
									'id' => '',
									...
									'comment' => '',
									'received_on' => '',
									'legacy_stripe_charge' => 'yes',
									'gateway_info' => $stripe_charge['id'],
								);
							}
						}
					} else if ( $resa_payment_info['stripe_charges'] == 'updated' ) {
						foreach ( $resa_payment_info['legacy_stripe_charges'] as $legacy_charge ) {
							$stripe_charges_amount += $legacy_charge['amount'];
						}
					}
				}
				if ( $stripe_charges_amount != $resa['paid'] ) {
					$initial_payments[] = array(
						'id' => '',
						'amount' => $resa['paid'] - $stripe_charges_amount,
						'refunded_amount' => 0,
						'payment_method' => '',
						'online' => 'yes',
						'comment' => '',
				...
				if ( $resa['paid'] >= $resa['amount_to_pay'] ) {
					if ( $resa['amount_to_pay'] > 0 ) {
						$initial_payments[] = array(
							'id' => '',
							'amount' => $resa['amount_to_pay'],
							'refunded_amount' => 0,
							'payment_method' => $resa['payment_gateway'],
							'online' => 'yes',
							'comment' => '',
							'received_on' => $resa['received_on'],
						);
					}
					if ( $resa['paid'] > $resa['amount_to_pay'] ) {
						$initial_payments[] = array(
							'id' => '',
							'amount' => $resa['paid'] - $resa['amount_to_pay'],
							'refunded_amount' => 0,
							'payment_method' => '',
							'online' => 'yes',
							'comment' => '',
		...
		$initial_payments = array_reverse( $initial_payments );
		$payments_logs = array();
		if ( $resa['payments_logs'] ) {
			$payments_logs = json_decode( $resa['payments_logs'], true );
			usort( $payments_logs, function( $a, $b ) {
				...
				return $b['received_on'] <=> $a['received_on'];
			});
			if ( ! $desc_order ) {
				$payments_logs = array_reverse( $payments_logs );
			}
			foreach ( $payments_logs as $payment_log_id => $payment_log ) {
				if ( ( $payment_log['online'] == 'yes' ) && $payment_log['received_on'] ) {
					$payments_logs[ $payment_log_id ]['received_on'] = $this->get_blog_datetime( $payment_log['received_on'] );
				}
				if ( $payment_log['refunded_amount'] > 0 ) {
					if ( $payment_log['refunded_amount'] < $payment_log['amount'] ) {
						$added_comment = '';
						if ( $payment_log['comment'] ) {
							$added_comment .= ' (';
						}
						$added_comment .= esc_html__( 'Partially refunded', 'hbook-admin' );
						$added_comment .= ' - ';
						$added_comment .= $this->price_with_symbol( $payment_log['refunded_amount'] );
						if ( $payment_log['comment'] ) {
							$added_comment .= ')';
						}
					} else {
						if ( $payment_log['comment'] ) {
							$added_comment = ' (' . esc_html__( 'Refunded', 'hbook-admin' ) . ')';
						} else {
							$added_comment = esc_html__( 'Refunded', 'hbook-admin' );
						}
					}
					$payments_logs[ $payment_log_id ]['comment'] = $payment_log['comment'] . $added_comment;
				}
			}
		}
		return array_merge( $payments_logs, $initial_payments );
	}
	...
	public function update_resa_emails_count() {
		if ( get_option( 'hb_update_resa_emails_count' ) == 'yes' ) {
			$this->hbdb->update_resa_emails_count();
		}
	}

	public function update_customer_resa_count() {
		if ( get_option( 'hb_update_customer_resa_count' ) == 'yes' ) {
	...
	private function get_translated_post_url_by_locale( $post_id, $locale ) {
		$translated_post_id = $this->get_translated_post_id_by_locale( $post_id, $locale );
		if ( function_exists( 'icl_object_id' ) && ! function_exists( 'pll_get_post' ) ) {
			$wpml_post_lang = apply_filters( 'wpml_post_language_details', NULL, $translated_post_id );
			$post_lang_code = $wpml_post_lang['language_code'];
			$url = apply_filters( 'wpml_permalink', get_permalink( $translated_post_id ), $post_lang_code );
		} else {
	...
	public function payment_gateways_support_later_payment() {
	...
	public function frontend_buttons_css() {
		if ( get_option( 'hb_buttons_style' ) == 'custom' && $this->load_css() ) {
			...
			$css_rules = '.hb-accom-listing-shortcode-wrapper input[type="submit"], .hbook-wrapper input[type="submit"], .hb-payment-link-form-wrapper input[type="submit"] { ' . $css_rules . '} ';
			...
			$css_rules .= '.hb-accom-listing-shortcode-wrapper input[type="submit"]:hover, .hbook-wrapper input[type="submit"]:hover, .hb-payment-link-form-wrapper input[type="submit"]:hover { background: ' . $rule_value . ' !important; }';
			echo( '<style type="text/css">' );
			echo( '.hb-accom-listing-shortcode-wrapper input[type="submit"], .hbook-wrapper input[type="submit"], .hb-payment-link-form-wrapper input[type="submit"] { border: none !important; cursor: pointer; transition: background 0.4s; } ' );
	...
	public function check_delete_uncompleted_resa() {
		$resa_types = array( 'parents', 'standalone' );
		foreach ( $resa_types as $resa_type ) {
			if ( $resa_type == 'parents' ) {
				$uncompleted_resa = $this->hbdb->get_uncompleted_parent_resa();
				$old_uncompleted_parent_resa_ids = $this->hbdb->get_old_uncompleted_parent_resa_ids();
			} else {
				$uncompleted_resa = $this->hbdb->get_uncompleted_resa();
				$old_uncompleted_resa_ids = $this->hbdb->get_old_uncompleted_resa_ids();
			}
			foreach ( $uncompleted_resa as $resa ) {
				$delete_uncompleted_resa = true;
				$payment_gateway = $this->get_payment_gateway( $resa['payment_gateway'] );
				if ( $payment_gateway ) {
					$check_result = $payment_gateway->check_uncompleted_resa( $resa );
					if ( $check_result != 'delete' ) {
						$delete_uncompleted_resa = false;
					}
				}
				if ( $delete_uncompleted_resa ) {
					if ( ( $resa_type == 'parents' ) && in_array( $resa['id'], $old_uncompleted_parent_resa_ids ) ) {
						$this->hbdb->delete_parent_resa( $resa['id'] );
					} else if ( ( $resa_type == 'standalone' ) && in_array( $resa['id'], $old_uncompleted_resa_ids ) ) {
	...
	public function export_resa() {
		if (
			...
			$data_to_export = array_merge(
				$this->get_exportable_resa_fields(),
				$this->get_exportable_additional_info_fields(),
				$this->get_exportable_extra_services_fields(),
				$this->get_exportable_fees_fields(),
				$this->get_exportable_customer_fields()
			);
			...
			foreach ( $resa as $resa_key => $resa_data ) {
				...
				if ( is_array( $resa_extra_services ) ) {
					foreach ( $resa_extra_services as $resa_extra_id => $resa_extra ) {
						if ( isset( $extras[ $resa_extra_id ] ) ) {
							...
							$extra_calculated_values = $this->calculate_fees_extras_values( $resa_data, 0, $resa_extra );
							$extra_services[ 'extra_' . $resa_extra_id . '_price' ] = $extra_calculated_values['price'];
						}
					}
				}
				...
				$resa_fees = array();
				if ( $resa_data['fees'] ) {
					$resa_fees = json_decode( $resa_data['fees'], true );
				}
				$fees = array();
				if ( is_array( $resa_fees ) ) {
					$resa_object = $this->resa->load( $resa_data['id'] );
					foreach ( $resa_fees as $resa_fee ) {
						$fees[ 'fee_' . $resa_fee['id'] ] = $this->resa->fee_price( $resa_fee['id'] );
					}
				}
				...
				$resa_payment_logs = $this->prepare_resa_payments_logs( $resa_data );
				$resa_payment_logs_export = array();
				foreach ( $resa_payment_logs as $payment ) {
					unset( $payment['id'] );
					unset( $payment['gateway_info'] );
					unset( $payment['online'] );
					array_walk(
						$payment,
						function ( &$value ) {
					...
					$resa_payment_logs_export[] = $payment;
				}
				$resa[ $resa_key ]['payments_history'] = str_replace( ',', ';', json_encode( $resa_payment_logs_export ) );
				$resa[ $resa_key ]['paid'] = $this->resa_total_paid( $resa[ $resa_key ] );

				$resa[ $resa_key ] = array_merge( $resa[ $resa_key ], $extra_services, $fees, $optional_info, $customer_info, $additional_info );
			}
	...
	public function open_document() {
		if (
			...
			if ( $doc ) {
				...
				if ( $this->email_doc_locale == 'en' ) {
					$this->email_doc_locale = 'en_US';
				}
				add_filter( 'locale', array( $this, 'email_doc_filter_locale' ) );
	...
	public function get_exportable_resa_fields() {
		return array(
			...
			'payments_history' => esc_html__( 'Payments history', 'hbook-admin'),
	...
	public function get_exportable_extra_services_fields() {
		...
		foreach ( $extras as $extra ) {
			...
			$exportable_extra[ 'extra_' . $extra['id'] . '_price' ] = $extra['name'] . ' (' . esc_html__( 'price', 'hbook-admin' ) . ')';
		}
		return $exportable_extra;
	}

	public function get_exportable_fees_fields() {
		$fees = $this->hbdb->get_all( 'fees' );
		$exportable_fees = array();
		foreach ( $fees as $fee ) {
			$exportable_fees[ 'fee_' . $fee['id'] ] = $fee['name'];
		}
		return $exportable_fees;
	}
	...
	public function get_blog_datetime( $datetime ) {
		...
		if ( empty( $tzstring ) && is_numeric( $offset ) && ( $offset != 0 ) ) {
			$offset *= -60;
			if ( $offset > 0 ) {
				$offset = '+' . $offset;
			}
			return date( 'Y-m-d H:i:s', strtotime( "$offset minutes", strtotime( $datetime ) ) );
		}
		if ( empty( $tzstring ) ) {
			$tzstring = 'UTC';
		}
	...
	public function get_hbook_pages() {
		// licence removed: always return full menu without licence entry
		return array(
			array(
				'id' => 'hb_reservations',
			...
			array(
				'id' => 'hb_text',
				'name' => esc_html__( 'Text/Translations', 'hbook-admin' ),
